          ...
          <AnimatePresence>
            {isDropdownOpen && (
              <motion.div 
                ...
                exit={{ opacity: 0, y: -10 }}
                className="absolute right-0 mt-2 w-56 bg-[#2a2a3c] border border-[#35354a] rounded-lg shadow-xl z-10 overflow-hidden"
              >
                {MODELS.map(model => (
                  <button
                    key={model}
        ...
        <div className="p-[2px] rounded-3xl animate-gradient">
          <div className="bg-[#1e1e2e] rounded-3xl flex items-end p-2">
            <textarea
              ...
              className="flex-1 bg-transparent border-none outline-none resize-none max-h-32 min-h-[44px] px-4 py-3 text-white placeholder-gray-400"
              rows={1}
            />
            <button 
              onClick={handleSend}
              disabled={!input.trim() || isTyping}
              className="p-3 bg-[#2a2a3c] hover:bg-[#35354a] disabled:opacity-50 disabled:hover:bg-[#2a2a3c] rounded-full transition-colors mb-1 mr-1"
            >
              <Send size={20} className={input.trim() && !isTyping ? "text-[#818cf8]" : "text-gray-500"} />
            </button>
          </div>
        </div>
      </footer>
